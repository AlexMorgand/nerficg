// Core torch API for the embedded FastGS rasterization (renamed to faster2dgs_core).
// Unqualified `forward`/`backward`/`inference` must be fully qualified: unqualified
// `forward` can resolve to std::forward from standard headers.
...
faster2dgs_core::rasterization::forward_wrapper(
    ...
    const bool proper_antialiasing)
{
    ...
    auto [n_instances, n_buckets, instance_primitive_indices_selector] = faster2dgs_core::rasterization::forward(
...
faster2dgs_core::rasterization::backward_wrapper(
    ...
    const int instance_primitive_indices_selector)
{
    ...
    faster2dgs_core::rasterization::backward(
...
torch::Tensor
faster2dgs_core::rasterization::inference_wrapper(
    const torch::Tensor& means,
    const torch::Tensor& scales,
    const torch::Tensor& rotations,
    const torch::Tensor& opacities,
    ...
    const bool to_chw)
{
    ...
    faster2dgs_core::rasterization::inference(
        resize_primitive_buffers,
        resize_tile_buffers,
        resize_instance_buffers,
        reinterpret_cast<float3*>(means.data_ptr<float>()),
