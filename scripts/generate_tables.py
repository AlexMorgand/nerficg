...
"""
generate_tables.py: Computes and tabulates image quality metrics for multiple scenes and methods.
...
Images can be in any format supported by our load_image function (only tested with PNG).
...
"""
known_metrics: dict[str, tuple[Callable, Callable, bool]] = {
...
def compute_metrics(
    ...
    metric_requires_mask: list[bool],
) -> list[float]:
...
def main(root_dir: Path, config_only: bool):
    ...
    Logger.log_info(f'reading config file from {config_path}')
    if not config_path.exists():
        Logger.log_info('config file not found -> creating default config file')
        config = write_empty_config_file(root_dir)
    else:
    ...
    for metric_name in metric_names:
        if metric_name not in known_metrics:
            Logger.log_error(f'requested unknown metric: {metric_name} -> exiting')
            return
    ...
    for scene_name in scene_names:
        Logger.log_info(f'processing scene {scene_name}')
        ...
        if mask_path.exists():
            ...
            if max(metric_requires_mask):
                Logger.log_warning(f'mask required for metric calculation but no mask images found for scene {scene_name} -> values will default to zero')
            mask_images = [None] * len(gt_images)
        for method_name in method_names:
            method_path = scene_path / method_name
            if not method_path.exists():
                Logger.log_warning(f'no results available for method {method_name} on scene {scene_name} -> filling with zeros')
                for metric_name in metric_names:
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        ...
        description='Computes and tabulates image quality metrics for multiple methods and scenes.'
    )
    parser.add_argument(
